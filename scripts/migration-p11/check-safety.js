...
const FIXTURE_MARKER = '.autoplan-p11-authorized-copy';
const FIXTURE_MANIFEST = 'p11-fixture-manifest.json';
...
function validateFixtureRoot(value) {
  ...
  let realRoot;
  try { realRoot = fs.realpathSync.native(root); } catch { return blocked('fixture_root_invalid'); }
  if (USER_DATA_PATH.test(realRoot)) return blocked('fixture_real_userdata_rejected');
  const canonicalRoot = path.resolve(realRoot);
  const expectedRoot = process.platform === 'win32' ? root.toLowerCase() : root;
  const actualRoot = process.platform === 'win32' ? canonicalRoot.toLowerCase() : canonicalRoot;
  if (actualRoot !== expectedRoot) return blocked('fixture_parent_symlink_rejected');
  ...
  if (!markerInfo?.isFile() || markerInfo.isSymbolicLink() || !manifestInfo?.isFile() || manifestInfo.isSymbolicLink() ||
      markerInfo.size > 1024 || manifestInfo.size > 64 * 1024) return blocked('fixture_authorization_missing');
  let descriptor;
  try { descriptor = JSON.parse(fs.readFileSync(manifest, 'utf8')); } catch { return blocked('fixture_manifest_invalid'); }
  if (descriptor?.schema_version !== 1 || descriptor?.kind !== 'p11-authorized-fixture' || descriptor?.authorized_copy !== true) {
...
module.exports = { FIXTURE_MANIFEST, FIXTURE_MARKER, createSafeEnvironment, inspectEvidenceFile, isWithin, sanitizeText, scanSensitiveText, sha256, validateFixtureRoot };

...
const FIXTURE_MARKER = '.autoplan-p12-authorized-copy';
const FIXTURE_MANIFEST = 'p12-fixture-manifest.json';
...
function validateFixtureRoot(value) {
  ...
  if (!markerInfo?.isFile() || markerInfo.isSymbolicLink() || markerInfo.size > 1024 ||
      !manifestInfo?.isFile() || manifestInfo.isSymbolicLink() || manifestInfo.size > 64 * 1024) return blocked('fixture_authorization_missing');
  let descriptor;
  try { descriptor = JSON.parse(fs.readFileSync(manifest, 'utf8')); } catch { return blocked('fixture_manifest_invalid'); }
  if (descriptor?.schema_version !== 1 || descriptor?.kind !== 'p12-authorized-process-fixture' || descriptor?.authorized_copy !== true) {

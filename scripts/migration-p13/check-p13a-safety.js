...
const FIXTURE_MARKER = '.autoplan-p13a-authorized-chat-copy';
const FIXTURE_MANIFEST = 'p13a-fixture-manifest.json';
...
function validateFixtureRoot(value) {
  ...
  if (normalize(root) !== normalize(actual)) return blocked('fixture_parent_symlink_rejected');
  ...
  if (!markerInfo?.isFile() || markerInfo.isSymbolicLink() || markerInfo.size > 1024 || !manifestInfo?.isFile() || manifestInfo.isSymbolicLink() || manifestInfo.size > 64 * 1024) {
  ...
  if (descriptor?.schema_version !== 1 || descriptor?.kind !== 'p13a-authorized-chat-fixture' || descriptor?.authorized_copy !== true || descriptor?.contains_real_userdata !== false) {
...
function createSafeEnvironment(temporaryRoot, source = process.env) {
  ...
  Object.assign(environment, {
    ...
    AUTOPLAN_P13A_VERIFICATION: '1', AUTOPLAN_SIDECAR_GO_CHAT_API: 'false',
  });
